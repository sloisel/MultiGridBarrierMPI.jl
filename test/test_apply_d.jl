...
Dz_native = Matrix(Dz_hpc)

# Compare with native
D_native = [SparseMatrixCSC(D) for D in D_op]
Dz_expected = hcat([D * z_native for D in D_native]...)

println(io0(), "[DEBUG] apply_D result size: $(size(Dz_hpc))")
println(io0(), "[DEBUG] apply_D row_partition: $(Dz_hpc.row_partition)")
println(io0(), "[DEBUG] apply_D col_partition: $(Dz_hpc.col_partition)")
...
result_mpi = MultiGridBarrier.map_rows((xi, qi) -> sum(qi.^2), x, Dz_hpc)
...
result2_mpi = MultiGridBarrier.map_rows((xi, qi) -> [qi[1]^2, qi[2]^2]', x, Dz_hpc)
