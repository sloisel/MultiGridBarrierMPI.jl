...
for L in [5, 6]
    ...
    # Create test vectors
    u_native = ones(n)
    u_mpi = LinearAlgebraMPI.VectorMPI(u_native; partition=w_mpi.partition)
    ...
    for _ in 1:N_ITER
        t = time_ns()
        result = LinearAlgebraMPI.map_rows(f_scalar, x_mpi, w_mpi)
        t = time_ns() - t
        push!(mpi_times, t)
    end
    ...
    println("\n4. Vector add (u + v):")
    v_native = 2 .* u_native
    v_mpi = LinearAlgebraMPI.VectorMPI(v_native; partition=w_mpi.partition)
    ...
    for _ in 1:N_ITER
        t = time_ns()
        result = LinearAlgebraMPI.map_rows(f_rowvec, x_mpi, w_mpi)
        t = time_ns() - t
        push!(mpi_rv, t)
    end
