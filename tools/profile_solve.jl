...
u_test_mpi = LinearAlgebraMPI.VectorMPI(u_test_native; partition=w_mpi.partition)
...
for _ in 1:N_ITER
    t = time_ns()
    result = LinearAlgebraMPI.map_rows(f_scalar3, x_mpi, w_mpi, u_test_mpi)
    t = time_ns() - t
    push!(mpi_times, t)
end
...
for _ in 1:N_ITER
    t = time_ns()
    result = LinearAlgebraMPI.map_rows(f_rowvec, x_mpi, w_mpi, u_test_mpi)
    t = time_ns() - t
    push!(mpi_rv_times, t)
end
